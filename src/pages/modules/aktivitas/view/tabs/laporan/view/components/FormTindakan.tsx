...
export function FormTindakan({
	...
	const { fields, append, remove } = useFieldArray({
		control,
		name: "tindakan_diharapkan" as never,
	});
		...
		<>
			<div className="grid grid-cols-[12rem_1fr] flex-row gap-5">
				<p className="text-neutral font-normal  text-sm">
					Tindakan Yang Diharapkan <span className={"text-red-500"}>*</span>
				</p>
				<div className="flex flex-col gap-2">
					{fields.map((field, index) => (
						<div key={field.id} className="flex items-center w-full gap-2">
							<TextInput
								form={form}
								name={`tindakan_diharapkan.${index}`}
								inputClassName="placeholder:text-[#888]/50"
								placeholder="Cth: Perbaikan segera oleh pemerintah desa atau pihak terkait"
								isDisabled={disabled}
								className="flex-1"
							/>
							<button
