...
export function generatePdfSurat(data: Surat) {
	...
	const tableBody = [
		[
			{
				text: surat?.warga_desa ? "NIK" : "NIK/Nomor Paspor",
				fontSize: 10,
			},
		...
		[
			{ text: "Tempat, Tanggal Lahir", fontSize: 10 },
			{
				text: `: ${surat?.tempat_lahir || "-"}, ${
					surat?.tanggal_lahir
						? dayjs(surat?.tanggal_lahir).locale("id").format("DD MMMM YYYY")
						: "-"
				}`,
				fontSize: 10,
			},
		...
		[
			...
			{
				text: `: ${surat?.jenis_kelamin === "L" ? "Laki-laki" : "Perempuan"}`,
				fontSize: 10,
			},
		],
		[
	...
	if (surat?.warga_desa) {
		...
		tableBody.push(
			[
				{ text: "Kewarganegaraan", fontSize: 10 },
				{
			...
			[
				{ text: "Alamat Sesuai Identitas / Paspor", fontSize: 10 },
				{ text: `: ${surat?.alamat_identitas || "-"}`, fontSize: 10 },
			],
			[
				{ text: "Alamat Tinggal Sekarang", fontSize: 10 },
				{ text: `: ${surat?.alamat || "-"}`, fontSize: 10 },
			],
			[
				{ text: "Jumlah Pengikut yang Tinggal", fontSize: 10 },
				{ text: `: ${surat?.jumlah_pengikut || 0} Orang`, fontSize: 10 },
			]
		);
	}
	...
	return {
		content: [
			...
			// ✅ 4. Pembuka
			{
				text: `Yang bertanda tangan di bawah ini, atas nama ${
					surat?.disahkan_oleh || "-"
				} ${profil_desa?.nama_organisasi || "-"} Kecamatan ${
					profil_desa?.Kecamatan || "-"
				} Kabupaten ${
					profil_desa?.Kabupaten || "-"
			...
			{
				table: {
					widths: surat?.warga_desa ? ["25%", "75%"] : ["33%", "67%"],
					body: tableBody,
				},
				layout: "noBorders",
				margin: [20, 0, 0, 10],
			...
			// ✅ 6. Paragraf Penutup
			{
				text: surat?.warga_desa
					? `Adalah benar warga ${profil_desa?.nama_organisasi} Kecamatan ${profil_desa?.Kecamatan} Kabupaten ${profil_desa?.Kabupaten} dan sebagaimana berdomisili di alamat yang telah diterangkan di atas.`
					: "Adalah benar warga pendatang yang baru berdomisili atau bertempat tinggal sementara sebagaimana di alamat yang telah diterangkan di atas.",
				fontSize: 10,
				margin: [0, 0, 0, 10],
			},
			{
				text: `Surat Keterangan Domisili ini diberikan ${
					surat?.keperluan || "-"
				}`,
				fontSize: 10,
				margin: [0, 0, 0, 10],
			...
			{
				text: `Demikian Surat Keterangan Domisili ini dibuat dan diberikan kepada yang bersangkutan untuk dipergunakan sebagaimana mestinya.`,
				fontSize: 10,
				margin: [0, 0, 0, 20],
			},
