...
export function generatePdfSurat(data: Surat) {
	...
	return {
		content: [
			...
			// ✅ 6) Paragraf penjelasan usaha
			{
				text: surat?.warga_desa
					? `Adalah benar warga ${
							profil_desa?.nama_organisasi || "-"
					  } Kecamatan ${profil_desa?.Kecamatan || "-"} Kabupaten ${
							profil_desa?.Kabupaten || "-"
					  } dan berdasarkan keterangan yang kami terima, yang bersangkutan memiliki usaha:`
					: `Adalah benar memiliki usaha di wilayah ${
							profil_desa?.nama_organisasi || "-"
					  } Kecamatan ${profil_desa?.Kecamatan || "-"} Kabupaten ${
							profil_desa?.Kabupaten || "-"
					  } dan sebagaimana berdomisili di alamat yang telah diterangkan di atas. Berdasarkan keterangan yang kami terima, yang bersangkutan memiliki usaha:`,
				fontSize: 10,
				margin: [0, 0, 0, 10],
			},
			...
			// ✅ 7) Tabel usaha
			{
				table: {
					...
					body: [
						[
							{ text: "Nama Usaha", fontSize: 10 },
							{ text: `: ${surat?.nama_usaha || "-"}`, fontSize: 10 },
						],
						[
							{ text: "Jenis Usaha", fontSize: 10 },
						...
						[
							{ text: "NPWP", fontSize: 10 },
							{ text: `: ${surat?.npwp || "-"}`, fontSize: 10 },
						],
						[
							{ text: "Alamat Usaha", fontSize: 10 },
							{ text: `: ${surat?.alamat_usaha || "-"}`, fontSize: 10 },
						],
						[
							{ text: "Usaha Berdiri Sejak", fontSize: 10 },
							{ text: `: ${surat?.usaha_berdiri_sejak || "-"}`, fontSize: 10 },
						],
					],
				},
				layout: "noBorders",
			...
			// ✅ 8) Penutup
			{
				text: `Surat Keterangan Usaha ini diberikan ${surat?.keperluan || "-"}`,
				fontSize: 10,
				margin: [0, 0, 0, 5],
			},
			{
				text: `Demikian Surat Keterangan Usaha ini dibuat dan diberikan kepada yang bersangkutan untuk dipergunakan sebagaimana mestinya.`,
				fontSize: 10,
				margin: [0, 0, 0, 20],
			},
