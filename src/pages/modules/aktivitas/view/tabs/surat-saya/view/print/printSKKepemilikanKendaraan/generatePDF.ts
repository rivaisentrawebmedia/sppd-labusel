...
export function generatePdfSurat(data: Surat) {
	...
	return {
		content: [
			...
			{
				text: `Yang bertanda tangan di bawah ini ${surat?.disahkan_oleh} ${profil_desa?.nama_organisasi} Kecamatan ${profil_desa?.Kecamatan} Kabupaten ${profil_desa?.Kabupaten} menerangkan bahwa berdasarkan keterangan dari :`,
				fontSize: 10,
				margin: [0, 0, 0, 5],
			},
			// 4️⃣ Table
			...
			{
				text: `Orang tersebut adalah penduduk ${profil_desa?.nama_organisasi}, yang mempunyai kendaraan dengan rincian sebagai berikut, sesuai BPKB :`,
				fontSize: 10,
				margin: [0, 0, 0, 5],
			},
			// 4️⃣ Table
			[
				{
					table: {
						...
						body: [
							[
								{ text: "Merk/Type", fontSize: 10 },
								{ text: `: ${surat?.merk || ""}`, fontSize: 10 },
							],
							[
								{ text: "Tahun Pembuatan", fontSize: 10 },
								{ text: `: ${surat?.tahun_pembuatan || ""}`, fontSize: 10 },
							],
							[
								{ text: "Warna", fontSize: 10 },
								{ text: `: ${surat?.warna || ""}`, fontSize: 10 },
							],
							[
								{ text: "Nomor Polisi", fontSize: 10 },
								{ text: `: ${surat?.nomor_polisi || ""}`, fontSize: 10 },
							],
							[
								{ text: "Nomor Mesin", fontSize: 10 },
								{ text: `: ${surat?.nomor_mesin || ""}`, fontSize: 10 },
							],
							[
								{ text: "Nomor Rangka", fontSize: 10 },
								{ text: `: ${surat?.nomor_rangka || ""}`, fontSize: 10 },
							],
							[
								{ text: "Nomor BPKB", fontSize: 10 },
								{ text: `: ${surat?.nomor_bpkb || ""}`, fontSize: 10 },
							],
							[
								{ text: "Bahan Bakar", fontSize: 10 },
								{ text: `: ${surat?.bahan_bakar || ""}`, fontSize: 10 },
							],
							[
								{ text: "Isi Silinder", fontSize: 10 },
								{ text: `: ${surat?.isi_silinder || ""}`, fontSize: 10 },
							],
							[
								{ text: "Atas Nama", fontSize: 10 },
								{ text: `: ${surat?.atas_nama || ""}`, fontSize: 10 },
							],
						],
					},
					layout: "noBorders",
