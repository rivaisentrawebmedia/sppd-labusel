...
export function generatePdfSurat(data: Surat) {
	...
	return {
		content: [
				...
				table: {
					...
					body: [
						...
						[
							...
							`: ${surat?.tempat_lahir}, ${dayjs(surat?.tanggal_lahir)
								.locale("id")
								.format("DD MMMM YYYY")}`,
						],
						[
			...
			// Paragraf transisi
			{
				text: `Adalah benar warga ${profilDesa?.nama_organisasi} Kecamatan ${profilDesa?.Kecamatan} Kabupaten ${profilDesa?.Kabupaten} dan berdasarkan keterangan, yang bersangkutan akan mengadakan:`,
				fontSize: 10,
				alignment: "justify",
				margin: [0, 0, 0, 10],
			},
			...
			// Tabel Detail Acara
			{
				...
				table: {
					widths: ["30%", "*"],
					body: [
						["Nama Kegiatan/Acara", `: ${surat?.nama_acara}`],
						["Tempat", `: ${surat?.tempat_acara}`],
						[
							"Hari, Tanggal",
							`: ${dayjs(surat?.tanggal).locale("id").format("dddd")}, ${dayjs(
								surat?.tanggal
							)
								.locale("id")
								.format("DD MMMM YYYY")}`,
						],
						["Waktu", `: ${surat?.dimulai} - ${surat?.selesai} WIB`],
						["Penanggung Jawab", `: ${surat?.penanggung_jawab}`],
						["Kontak", `: ${surat?.kontak}`],
					],
				},
				layout: "noBorders",
			...
			// Paragraf syarat
			{
				text: `Surat ini diberikan sebagai rekomendasi izin keramaian ke Polsek Pasir Gunung Selatan, dengan ketentuan-ketentuan :`,
				fontSize: 10,
				alignment: "justify",
				margin: [0, 0, 0, 10],
			},
			...
			{
				ol: [
					"Pada waktu pelaksanaan kegiatan keramaian harus dapat menjaga kebersihan, ketentraman, dan ketertiban lingkungan, menghargai waktu ibadah, dan menciptakan kerukunan umat beragama, sampai selesai kegiatan keramaian.",
					"Tidak diperkenankan/ dilarang melakukan hal-hal yang bertentangan dengan adat-istiadat setempat atau melakukan pesta minuman keras, dan melapor ke pihak berwenang bila terdapat kejadian yang meresahkan.",
				],
				fontSize: 10,
				margin: [0, 0, 0, 10],
			},
			...
			// Penutup
			{
				text: `Demikian Surat Rekomendasi Keramaian ini dibuat dan diberikan kepada yang bersangkutan untuk dipergunakan sebagaimana mestinya.`,
				fontSize: 10,
				alignment: "justify",
				margin: [0, 0, 0, 20],
			},
			...
			},
			{
				...
				columns: [
					// ➤ KIRI: Pelapor
					{
						...
						stack: [
							{
								text: `Bhabinkamtibmas ${profilDesa?.nama_organisasi}`,
								fontSize: 10,
								margin: [0, 0, 0, 50],
							},
							{
