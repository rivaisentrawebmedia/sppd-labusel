...
export function generatePdfSurat(data: Surat) {
	...
	return {
		content: [
			...
			{
				margin: [30, 0, 0, 10], // pl-32 = indent left
				table: {
					widths: ["30%", "*"],
					body: [
						["NIK", `: ${surat?.nik}`],
			...
			{
				text: `Adalah benar warga ${profilDesa?.nama_organisasi} Kecamatan ${profilDesa?.Kecamatan} Kabupaten ${profilDesa?.Kabupaten} dan berdasarkan laporan yang diterima, yang bersangkutan telah kehilangan:`,
				margin: [0, 0, 0, 10],
				fontSize: 10,
				alignment: "justify",
			},
			{
				...
				table: {
					widths: ["30%", "*"],
					body: [
						["Jenis Barang", `: ${surat?.jenis_barang}`],
						["Ciri-ciri Barang", `: ${surat?.ciri}`],
						["Tempat Kehilangan", `: ${surat?.tempat_kehilangan}`],
						[
							"Waktu Kehilangan",
							`: ${dayjs(surat?.waktu_kehilangan)
								.locale("id")
								.format("DD MMMM YYYY")}`,
						],
					],
			...
			{
				text: `Demikian Surat Pengantar Kehilangan ini dibuat dan diberikan kepada yang bersangkutan untuk dilaporkan ke kepolisian dan dipergunakan sebagaimana mestinya.`,
				margin: [0, 0, 0, 20],
				fontSize: 10,
				alignment: "justify",
			},
