...
export function generatePdfSurat(data: Surat) {
	...
	return {
		content: [
				...
				table: {
					...
					body: [
						...
						[
							...
							{
								text: `: ${surat?.tempat_lahir}, ${dayjs(surat?.tanggal_lahir)
									.locale("id")
									.format("DD MMMM YYYY")}`,
								fontSize: 10,
							},
			...
			// ✅ Paragraf penjelas tujuan
			{
				text: `Adalah benar warga ${profilDesa?.nama_organisasi} Kecamatan ${profilDesa?.Kecamatan} Kabupaten ${profilDesa?.Kabupaten} dan berdasarkan keterangan yang kami terima, yang bersangkutan akan bepergian ke:`,
				fontSize: 10,
				margin: [0, 0, 0, 10],
			},

			// ✅ Table Tujuan Perjalanan
			{
				table: {
					widths: ["30%", "70%"],
					body: [
						[
							{ text: "Tempat Tujuan", fontSize: 10 },
							{ text: `: ${surat?.tempat_tujuan}`, fontSize: 10 },
						],
						[
							{ text: "Maksud Tujuan", fontSize: 10 },
							{ text: `: ${surat?.maksud_tujuan}`, fontSize: 10 },
						],
						[
							{ text: "Lamanya", fontSize: 10 },
							{ text: `: ${surat?.lama} ${surat?.satuan_lama}`, fontSize: 10 },
						],
						[
							{ text: "Tanggal Keberangkatan", fontSize: 10 },
							{
								text: `: ${dayjs(surat?.tanggal_keberangkatan)
									.locale("id")
									.format("DD MMMM YYYY")}`,
								fontSize: 10,
							},
						...
						[
							{ text: "Jumlah Pengikut", fontSize: 10 },
							{ text: `: ${surat?.jumlah_pengikut} Orang`, fontSize: 10 },
						],
					],
				},
				layout: "noBorders",
			...
			// ✅ Penutup
			{
				text: `Surat Keterangan Bepergian ini diberikan ${surat?.keperluan}`,
				fontSize: 10,
				margin: [0, 0, 0, 5],
			},
			{
				text: `Demikian Surat Keterangan Bepergian ini dibuat dan diberikan kepada yang bersangkutan untuk dipergunakan sebagaimana mestinya.`,
				fontSize: 10,
				margin: [0, 0, 0, 20],
			},
