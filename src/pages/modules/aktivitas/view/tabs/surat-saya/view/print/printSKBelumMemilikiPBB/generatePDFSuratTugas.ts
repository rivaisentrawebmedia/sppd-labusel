...
export function generatePdfSurat(data: Surat) {
	...
	return {
		content: [
			...
			// 5️⃣ Isi penutup
			{
				text: `Nama tersebut di atas adalah Penduduk ${profil_desa?.nama_organisasi} Kecamatan ${profil_desa?.Kecamatan} Kabupaten ${profil_desa?.Kabupaten}, menerangkan bahwa nama di atas belum memiliki Pajak Bumi dan Bangunan (PBB) di ${profil_desa?.nama_organisasi}`,
				fontSize: 10,
				margin: [0, 0, 0, 10],
			},

			{
				text: `Demikian Surat Keteranagan ini diperbuat dengan sebenarnya untuk dipergunakan sebagaimana mestinya.`,
				fontSize: 10,
				margin: [0, 0, 0, 20],
			},
