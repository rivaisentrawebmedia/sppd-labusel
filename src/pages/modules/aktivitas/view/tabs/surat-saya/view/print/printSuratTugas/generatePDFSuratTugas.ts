...
export function generatePdfSurat(data: Surat) {
	...
	const penerimaList = surat?.penerima ?? [];
	...
	return {
		content: [
			...
			{
				text: `Yang bertanda tangan di bawah ini, atas nama ${surat?.disahkan_oleh} ${profilDesa?.nama_organisasi} Kecamatan ${profilDesa?.Kecamatan} Kabupaten ${profilDesa?.Kabupaten} memberikan tugas/perintah kepada:`,
				fontSize: 10,
				margin: [0, 0, 0, 10],
			},

			// List penerima tugas (loop)
			...penerimaList.map((item) => ({
				margin: [30, 0, 0, 10],
				stack: [
					{
						columns: [
							{ width: "25%", text: "NIK", fontSize: 11 },
							{ width: "5%", text: ":", fontSize: 11 },
							{ width: "*", text: item.nik || "-", fontSize: 11 },
						],
						columnGap: 10,
					},
					{
						columns: [
							{ width: "25%", text: "Nama", fontSize: 11 },
							{ width: "5%", text: ":", fontSize: 11 },
							{ width: "*", text: item.nama || "-", fontSize: 11 },
						],
						columnGap: 10,
					},
					{
						columns: [
							{ width: "25%", text: "Jabatan", fontSize: 11 },
							{ width: "5%", text: ":", fontSize: 11 },
							{ width: "*", text: item.jabatan || "-", fontSize: 11 },
						],
						columnGap: 10,
					},
				],
			...
			// Penutup
			{
				text: "Demikian Surat Tugas ini dibuat dan diberikan kepada yang bersangkutan untuk dilaksanakan dengan penuh rasa tanggung jawab dan dipergunakan sebagaimana mestinya.",
				fontSize: 10,
				margin: [0, 0, 0, 20],
			},
