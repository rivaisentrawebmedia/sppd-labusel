	...
	return {
		content: [
			...
			// Tabel Identitas
			{
				table: {
					...
					body: [
						...
						["Agama", `: ${convertFromSnakeCase(surat?.agama)}`],
			...
			// Paragraf Penjelas
			{
				...
				} dan berdasarkan ${convertFromSnakeCase(surat?.dasar_pengajuan)} ${
					surat?.nomor_pengajuan
				} hingga dikeluarkannya surat keterangan ini, benar adanya bahwa status yang bersangkutan adalah:`,
				fontSize: 10,
				margin: [0, 0, 0, 10],
			},

			// Status (center, bold, uppercase)
			{
				text: `${
					surat?.jenis_kelamin === "L" ? "DUDA" : "JANDA"
				} ${convertFromSnakeCase(surat?.penyebab)?.toUpperCase()}`,
				fontSize: 10,
				alignment: "center",
				bold: true,
				margin: [0, 0, 0, 10],
			...
			// Keperluan
			{
				text: `Surat Keterangan Janda/Duda ini diberikan ${surat?.keperluan}`,
				fontSize: 10,
				margin: [0, 0, 0, 20],
			},
			...
			// Penutup
			{
				text: `Demikian Surat Keterangan Janda/Duda ini dibuat dan diberikan kepada yang bersangkutan untuk dipergunakan sebagaimana mestinya.`,
				fontSize: 10,
			},
